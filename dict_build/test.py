...
if T:
   print("----------------------- vrf dict build")
   vrf_dict = {}
   for vrf_obj in parse.find_parent_objects(['^ip vrf' ]):
           vrf_name = vrf_obj.re_match_iter_typed("ip vrf\s+(\S.*)")
           vrf_dict[vrf_name] = vrf_obj
   

   for k in vrf_dict:
      print(f"{k} : {vrf_dict[k].text}")
...
if T:
   ...
   for intf_obj in parse.find_parent_objects(['^vlan' ]):
           ...
           for vlan in vlans:
              if '-' in vlan:
                  pass
                  start_end_vlan = vlan.split('-')
                  _start_vlan = int(start_end_vlan[0])
                  _end_vlan = int(start_end_vlan[1])
                  #print(_start_vlan, _end_vlan)
                  for vlan in range(_start_vlan, _end_vlan,1):
                      vlan_int_list.append(vlan)
                      vlan_str_list.append(str(vlan))


              else:
                  vlan_int_list.append(int(vlan))
                  vlan_str_list.append(vlan)
...
if T:
   print("----------------------- vrf route dict build")
   vrf_route_dict = {}
   for intf_obj in parse.find_parent_objects(['^ip route vrf' ]):
           vrf_route = intf_obj.re_match_iter_typed("ip route vrf\s+(\S.*)")
           m = re.match(r'(\S+)\s+([0-9\. ]+)', vrf_route)
           vrf_name = m.groups()[0]
           route    = m.groups()[1]
           if vrf_name in vrf_route_dict:
               vrf_route_dict[vrf_name].append(route)
           else:
               vrf_route_dict[vrf_name] = []
               vrf_route_dict[vrf_name].append(route)
   
   for k in vrf_route_dict:
      print(f"{k} : {vrf_route_dict[k]}")
